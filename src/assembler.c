...
void handle_directive(char *directive, char *args_str, char args[MAX_ARGS][128], int arg_count, bool write_mode) {
    ...
    if (!strcmp(directive, ".asciz")) {
        ...
        if (start && end && start != end) {
            ...
            emit_bytes(NULL, 1, write_mode); // Null terminator
        }
        return;
    }
}
...
void process_line(char *line, bool write_mode) {
    ...
    if (!strcmp(ins, "la") || !strcmp(ins, "call")) {
        ...
        if (write_mode) {
            ...
            uint32_t bin1 = is_call ? encode_U_type(0x17, 1,  upper << 12) : encode_U_type(0x37, v1, upper << 12);
            uint32_t bin2 = is_call ? encode_I_type(0x67, 0x0, (int)lower, 1,  1) : encode_I_type(0x13, 0x0, (int)lower, v1, v1);
            emit_bytes(&bin1, 4, true); emit_bytes(&bin2, 4, true);
            return;
        }
    ...
    if (ins[0] == 'b') {
        if (try_parse_reg(args[0], &v1) == 0) v1 = eval_math(args[0], get_virtual_address(), false);
        char *target_name;
        if (!strcmp(ins, "beqz") || !strcmp(ins, "bnez") || !strcmp(ins, "bgtz") || !strcmp(ins, "blez")) {
            target_name = args[1];
        } else {
    ...
    uint32_t bin = encode_instruction(ins, v1, v2, v3);
    if (bin != 0) emit_bytes(&bin, 4, write_mode);
}
...
void save_elf(const char *filename) {
    ...
    char *strtab = calloc(1, 65536); int strtab_size = 1; 

    Elf32_Sym symtab[MAX_SYMBOLS + 1]; memset(symtab, 0, sizeof(symtab));
    int sym_count = 1;         
    int elf_sym_map[MAX_SYMBOLS]; memset(elf_sym_map, 0, sizeof(elf_sym_map));
        ...
        if (!(symbol_table[i].flags & (SYM_FLAG_GLOBAL | SYM_FLAG_EXTERN))) {
            ...
            if (symbol_table[i].address == 0xFFFFFFFF) {
                ...
                if (symbol_table[i].section == SECTION_ABS) {
                    symtab[sym_count].st_value = symbol_table[i].address;
                    symtab[sym_count].st_shndx = SHN_ABS;
                } else if (symbol_table[i].section == SECTION_DATA) {
                    symtab[sym_count].st_value = symbol_table[i].address - (as_state.origin + as_state.final_text_size);
                    symtab[sym_count].st_shndx = 2; // .data index
                } else {
                    symtab[sym_count].st_value = symbol_table[i].address - as_state.origin;
                    symtab[sym_count].st_shndx = 1; // .text index
                }
            }
        ...
        if (symbol_table[i].flags & (SYM_FLAG_GLOBAL | SYM_FLAG_EXTERN)) {
            ...
            if (symbol_table[i].address == 0xFFFFFFFF) {
                ...
                if (symbol_table[i].section == SECTION_ABS) {
                    ...
                    symtab[sym_count].st_value = symbol_table[i].address - (as_state.origin + as_state.final_text_size);
                    symtab[sym_count].st_shndx = 2; 
                    st_type = STT_OBJECT; // Tell GCC this is a writable variable
                } else {
                    symtab[sym_count].st_value = symbol_table[i].address - as_state.origin;
                    symtab[sym_count].st_shndx = 1; 
                    st_type = STT_FUNC;   // Tell GCC this is an executable function
                }
            }
            symtab[sym_count].st_info = ELF32_ST_INFO(STB_GLOBAL, st_type);
            elf_sym_map[i] = sym_count;
            sym_count++;
        }
    }
    ...
    ehdr.e_ident[EI_CLASS]   = ELFCLASS32; ehdr.e_ident[EI_DATA] = ELFDATA2LSB; ehdr.e_ident[EI_VERSION] = EV_CURRENT;
    ehdr.e_type = ET_REL; ehdr.e_machine = EM_RISCV; ehdr.e_version = EV_CURRENT;
    ehdr.e_ehsize = sizeof(Elf32_Ehdr); ehdr.e_shentsize = sizeof(Elf32_Shdr);
    ehdr.e_shnum = num_sections; ehdr.e_shoff = off_shdr; ehdr.e_shstrndx = 3;

    Elf32_Shdr shdr[7]; memset(shdr, 0, sizeof(shdr));
    ...
    shdr[1].sh_size      = as_state.text_size;
    ...
    shdr[2].sh_addr      = 0; 
    shdr[2].sh_offset    = off_data;
    shdr[2].sh_size      = as_state.data_size;
    shdr[2].sh_addralign = 4;

    shdr[3].sh_name      = 13;
    shdr[3].sh_type      = SHT_STRTAB;
    shdr[3].sh_offset    = off_shstrtab;
    shdr[3].sh_size      = shstrtab_size;
    shdr[3].sh_addralign = 1;
    
    shdr[4].sh_name      = 23;
    shdr[4].sh_type      = SHT_SYMTAB;
    shdr[4].sh_offset    = off_symtab;
    shdr[4].sh_size      = sym_count * sizeof(Elf32_Sym);
    shdr[4].sh_link      = 5;                 
    shdr[4].sh_info      = local_sym_count; 
    shdr[4].sh_entsize   = sizeof(Elf32_Sym);
    shdr[4].sh_addralign = 4;
    
    shdr[5].sh_name      = 31;
    shdr[5].sh_type      = SHT_STRTAB;
    shdr[5].sh_offset    = off_strtab;
    shdr[5].sh_size      = strtab_size;
    shdr[5].sh_addralign = 1;
    
    shdr[6].sh_name      = 39;
    shdr[6].sh_type      = SHT_RELA;
    shdr[6].sh_offset    = off_rela;
    shdr[6].sh_size      = rela_size;
    shdr[6].sh_link      = 4;   
    shdr[6].sh_info      = 1;   
